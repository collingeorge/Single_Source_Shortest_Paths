...
class BlockBasedDS:
    ...
    def batch_prepend(self, pairs):
        ...
        new_blocks = []
        for i in range(0, len(pairs), self.M // 2):
            block = pairs[i:i + self.M // 2]
...
def find_pivots(B, S, graph, bd, pred, k):
    W = set(S)
    W_layers = [set(S)]
    for _ in range(k):
        W_next = set()
...
def base_case(B, S, graph, bd, pred, k):
    ...
    return max_dist, [v for v in U0 if bd[v] < max_dist]

def bmss_p(l, B, S, graph, bd, pred, k, t):
    if l == 0:
        return base_case(B, S, graph, bd, pred, k)
    P, W = find_pivots(B, S, graph, bd, pred, k)
    M = 2 ** ((l - 1) * t)
    D = BlockBasedDS(M, B)
    for x in P:
        D.insert(x, bd[x])
